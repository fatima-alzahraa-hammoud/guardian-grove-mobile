...
class AddMemberScreen extends StatefulWidget {
  ...
  const AddMemberScreen({super.key, this.fromProfile = false});
...
class _AddMemberScreenState extends State<AddMemberScreen> {
  ...
  void _addAnotherMember() async {
    if (!_isFormValid()) return;

    try {
      // Create member data object with correct field names for backend
      final memberData = {
        'name':
            _nicknameController.text
                .trim(), // Backend expects 'name' not 'nickname'
        'birthday': _selectedBirthday!.toIso8601String(),
        'gender': _selectedGender.toLowerCase(),
        'role': _memberType.toLowerCase(), // 'child' or 'parent'
        'avatar': _selectedAvatar,
        'interests': _memberType == 'Child' ? _selectedInterests : [],
      };

      debugPrint(
        '🚀 Adding family member: ${memberData['name']} (${memberData['role']})',
      );

      // Add member to family via AuthBloc
      context.read<AuthBloc>().add(AddFamilyMemberEvent(memberData));
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  Expanded(
                    child: Text('Failed to add member: ${state.message}'),
                  ),
                ],
              ),
              backgroundColor: const Color(0xFFE53E3E),
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ], // Close action buttons children
                    ], // Close main content children
